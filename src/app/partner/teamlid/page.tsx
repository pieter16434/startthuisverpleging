...
function TeamllidContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/partner/team/onboarding?token=${token}`)
      .then(r => r.json())
      .then(d => {
        if (d.error) setTokenError(d.error)
        else setPartnerName(d.partner?.business_name ?? '')
        setValidating(false)
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/partner/team/onboarding', {
        method: 'POST',
    ...
    <div style={{ minHeight: '100vh', background: '#F1ECE0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '"Bricolage Grotesque", system-ui, sans-serif', padding: '32px 24px' }}>
      <div style={{ width: '100%', maxWidth: 440 }}>
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <p style={{ color: '#6E6B62', fontSize: 13, marginTop: 6 }}>Teamlid registreren</p>
        </div>
  ...
  if (done) return shell(
    <>
      ...
      <p style={{ color: '#6E6B62', fontSize: 15, lineHeight: 1.6 }}>Je account is gekoppeld aan <strong>{partnerName}</strong>. Je wordt doorgestuurd naar de inlogpagina…</p>
    </>
  )
    ...
    <>
      {partnerName && (
        <div style={{ background: '#2A3D2E', borderRadius: 10, padding: '12px 16px', marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 700, color: '#fff', margin: 0 }}>{partnerName}</p>
        </div>
      )}
      ...
      <p style={{ color: '#6E6B62', fontSize: 14, marginBottom: 24, lineHeight: 1.6 }}>
        Vul je gegevens in. Je kan codes verifiëren via het partnerportaal.
      </p>
      ...
      <p style={{ textAlign: 'center', fontSize: 13, color: '#8A9588', marginTop: 16 }}>
        Na het aanmaken log je in via <a href="/partner" style={{ color: '#B65436' }}>het partnerportaal</a>.
      </p>
    </>
  )
}

export default function TeamllidPage() {
  return (
    <Suspense fallback={
